	...
	private void cargarTabla(int idProyecto) {
		try {
			...
			for (Tarea_proyecto t : lista) {
				modeloTabla.addRow(new Object[] { t.getId_tarea_proyecto(), t.getProyec().getNombre(),
						t.getNombre_visible(), t.getCatalog().getNombre(), t.getEstadotar().getNombre(), "Asignar"
						// t.isActiva()
				});
			}
			...
			if (proyectoSeleccionado != null && proyectoSeleccionado.getId_proyecto() != -1) {
				...
				vista.getTable().getColumnModel().getColumn(5).setCellEditor(new DefaultCellEditor(comboTrabajadores));
